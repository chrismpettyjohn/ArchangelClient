...
export class WeaponUpdateComposer implements IMessageComposer<ConstructorParameters<typeof WeaponUpdateComposer>> {
    ...
    constructor(weaponId: number, uniqueName: string, displayName: string, minDamage: number, maxDamage: number, rangeInTiles: number, accuracy: number, reloadTime: number, reloadMessage: string, ammoCapacity: number, weight: number, cooldownSeconds: number, equipEffect: number, equipMessage: number, unequipMessage: string, attackMessage: string) {
