...
export class WeaponCreateComposer implements IMessageComposer<ConstructorParameters<typeof WeaponCreateComposer>> {
    ...
    constructor(uniqueName: string, displayName: string, minDamage: number, maxDamage: number, rangeInTiles: number, accuracy: number, reloadTime: number, reloadMessage: string, ammoCapacity: number, weight: number, cooldownSeconds: number, equipEffect: number, equipMessage: number, unequipMessage: string, attackMessage: string) {
